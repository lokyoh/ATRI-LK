...
async def _(bot_id: str | None = None) -> Result[list[BaseInfo]]:
    ...
    try:
        ...
        log.error(f"{router.prefix}/get_base_info 调用错误:{str_traceback(e)}")
...
async def _(bot_id: str | None = None) -> Result[QueryCount]:
    try:
        ...
        log.error(f"{router.prefix}/get_all_chat_count 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(bot_id: str | None = None) -> Result[QueryCount]:
    try:
        ...
        log.error(f"{router.prefix}/get_all_call_count 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(bot_id: str) -> Result[dict[str, int]]:
    try:
        ...
        data = {
            "friend_count": len(await BotUtils.get_friend_list(bot)),
            "group_count": len(await BotUtils.get_group_list(bot)),
        }
        return Result.ok(data, "拿到信息啦!")
    except (ValueError, KeyError):
        return Result.warning_("指定Bot未连接...")
    except Exception as e:
        log.error(f"{router.prefix}/get_fg_count 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(
    ...
    try:
        ...
        log.error(f"{router.prefix}/get_active_group 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(
    ...
    try:
        ...
        log.error(f"{router.prefix}/get_hot_plugin 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: BotStatusParam):
    try:
        if await BotManager.get_bot_status(param.bot_id) != param.status:
            await BotManager().toggel_bot_status(param.bot_id)
...
async def _() -> Result[BotBlockModule]:
    try:
        return Result.ok(await ApiDataSource.get_bot_block_module(), "拿到信息啦!")
    except Exception as e:
        log.error(f"{router.prefix}/get_bot_block_module 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")


@router.post(
    "/update_service_manage",
    dependencies=[authentication()],
    response_model=Result,
    response_class=JSONResponse,
    description="修改bot全局开关",
...
async def _(param: BotManageUpdateParam):
    try:
        module_data = await ApiDataSource.get_bot_block_module()
        for s in param.block_plugins:
            if s in module_data.block_plugins:
                module_data.all_plugins.remove(s)
            else:
                BotManager().toggle_global_service(s)
        for s in module_data.block_plugins:
            BotManager().toggle_global_service(s)
        for t in param.block_tasks:
            if t in module_data.block_tasks:
                module_data.block_tasks.remove(t)
            else:
                SchedulerController.get_job_by_id(t).pause()
        for t in module_data.block_tasks:
            SchedulerController.get_job_by_id(t).resume()
        return Result.ok()
    except Exception as e:
        log.error(f"{router.prefix}/update_bot_manage 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def system_logs_realtime(websocket: WebSocket, sleep: int = 5):
    ...
    log.debug("ws system_status is connect")
