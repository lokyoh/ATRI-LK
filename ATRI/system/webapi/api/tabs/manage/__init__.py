...
async def _(bot_id: str) -> Result:
    ...
    try:
        ...
        group_list = await BotUtils.get_group_list(bot)
        for g in group_list:
            ava_url = GROUP_AVA_URL.format(g.group_id, g.group_id)
            group_list_result.append(
                GroupResult(
                    group_id=str(g.group_id), group_name=g.group_name, ava_url=ava_url
                )
            )
        ...
        log.error(f"{router.prefix}/get_group_list 调用错误:{str_traceback(e)}")
...
async def _(group: UpdateGroup) -> Result[str]:
    try:
        ...
        log.error(f"{router.prefix}/update_group 调用错误:{str_traceback(e)}")
...
async def _(bot_id: str) -> Result[list[Friend]]:
    try:
        ...
        for f in friend_list:
            ...
            result_list.append(
                Friend(
                    user_id=str(f.user_id), nickname=f.nickname or "", ava_url=ava_url
                )
            )
        return Result.ok(
            result_list,
...
async def _() -> Result[dict[str, int]]:
    try:
        ...
        log.error(f"{router.prefix}/get_request_count 调用错误:{str_traceback(e)}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[ReqResult]:
    try:
        ...
        log.error(f"{router.prefix}/get_request_list 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: LeaveGroup) -> Result:
    try:
        ...
        group_list = await BotUtils.get_group_list(bot)
        if str(param.group_id) not in [str(g.group_id) for g in group_list]:
        ...
        log.error(f"{router.prefix}/leave_group 调用错误:{str_traceback(e)}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: DeleteFriend) -> Result:
    try:
        ...
        log.error(f"{router.prefix}/delete_friend 调用错误:{str_traceback(e)}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(bot_id: str, user_id: str) -> Result[UserDetail]:
    try:
        ...
        log.error(f"{router.prefix}/get_friend_detail 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(group_id: str) -> Result[GroupDetail]:
    try:
        ...
        log.error(f"{router.prefix}/get_group_detail 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: SendMessageParam) -> Result:
    try:
        ...
        await BotUtils.send_message(
            bot, "WebAPI", param.message, param.user_id, param.group_id
        )
        return Result.ok("发送成功!")
    except (ValueError, KeyError):
        return Result.warning_("指定Bot未连接...")
    except Exception as e:
        log.error(f"{router.prefix}/send_message 调用错误:{str_traceback(e)}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
