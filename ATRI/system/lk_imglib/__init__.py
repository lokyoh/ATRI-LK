...
@global_tu_add.got('img_path', '要添加在哪里呢，请输入"图名"或者"库名 图名"添加进指定库中')
...
async def _(event: MessageEvent, text: str = ArgPlainText("img_path")):
    ...
    try:
        ...
        image_manager.save_image(img_name=img_name, img_data=resp.content, libname=libname)
...
async def _(libname: str = ArgPlainText("libname"), img_name: str = ArgPlainText("img_name")):
...
async def _(event: GroupMessageEvent, libname: str = ArgPlainText("libname"), img_name: str = ArgPlainText("img_name")):
    img = GroupImageManager(str(event.group_id)).get_image_from(libname=libname, img_name=img_name)
    if img is not None:
...
rand_global_img = plugin.cmd_as_group('随机图片', '从全局获取随机一张图片,当存在参数时指定图库名')
...
rand_group_img = plugin.on_command('/图库.随机本群图片', '从本群获取随机一张图片,当存在参数时指定图库名',
                                   aliases={'/图片'})
