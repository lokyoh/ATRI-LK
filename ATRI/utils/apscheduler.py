...
class SchedulerJob:
    ...
    def status(self):
        if hasattr(self.job, "next_run_time"):
            status = (
                "next run at: " + self.job.next_run_time.strftime("%Y-%m-%d %H:%M:%S %Z")
...
class SchedulerController:
    ...
    def add_job(self, func, name: str, trigger: str | BaseTrigger = 'date', args: list | None = None,
                kwargs: dict | None = None, **job_kwargs) -> SchedulerJob:
            ...
            if inspect.iscoroutinefunction(f):
                async def wrapper(*args, **kwargs):
                    try:
                        ...
                        log.error(f'在执行`{self.service}`的任务`{name}`时失败:\n{str_traceback(e)}')
        ...
        job = SchedulerJob(func=job_func(func), trigger=trigger, id=f'{self.service}-{name}', **job_kwargs)
        self.service_schedulers[self.service][name] = job
        return job

    def add_pause_job(self, func, name: str, trigger: str | BaseTrigger = 'date', **kwargs) -> SchedulerJob:
        """添加暂停的计划任务"""
    ...
    def has_job(self, name):
        """判断是否存在指定Job"""
        return self.service in self.service_schedulers and name in self.service_schedulers[self.service]
